...
data Symbol
  = LCB | RCB | LP | RP | LB | RB
  | POINT | COMMA | SEMICOLON | PLUS
  | MINUS | ASTERISK | SLASH | AMPERSAND
  | BAR | LESS_THAN | GREATER_THAN | EQUAL | TILDE
  deriving (Show, Eq)

parseFile :: FilePath -> IO [Token]
parseFile path = do
  contents <- readFile path
  return . map parseTokens . concat . map words_ . sus . seperateStrings . seperateSymbols . stripComments $ contents 
